...
- `create-release.yml` — queues releases per repository and creates/reuses exact tags plus published releases before moving eligible stable channels. The validated `version` is authoritative; `is-prerelease` is deprecated and ignored. Conflicting state fails closed, while the final branch/tag snapshot prevents stale or superseded runs from moving floating/latest channels or returning `is-latest`. See `README.md` for the public contract and failure semantics.
