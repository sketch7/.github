...
- `create-release.yml` — serializes releases per repository, creates/reuses immutable exact tag and published-release evidence, then moves eligible stable channels. `tag-tmpl` supplies literal text on both sides of its single `{major}` marker: exact tags substitute the full version and floating tags substitute the major; invalid or leading-dash results fail validation. Release inspection precedes tag mutation, treats only API 404 as absent, and fails closed for drafts, prerelease mismatches, conflicting/non-commit exact tags, malformed responses, or other API errors. Immediately before channel mutation, the workflow re-reads the triggering branch head and paginates stable tags again, requiring its SHA to match and no higher stable exact version to exist in that major. Both highest-stable-major classification and same-major mutation eligibility come from this final paginated snapshot. Stable mutation requires a branch ref; known callers use branch pushes or branch-selected manual dispatches, while tag-triggered stable calls fail closed. Matching stale reruns remain successful but skip latest/floating mutations. New stable releases start non-latest; effective `is-latest` requires the final highest-stable-major classification plus current branch-head and same-major mutation eligibility, so stale/superseded runs cannot trigger bump-main. Pre-release and floating tags never participate in stable-version comparisons.
